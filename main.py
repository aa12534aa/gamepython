...
for x in range(GRID_SIZE):
    for y in range(GRID_SIZE):
        if x == 0 or y == 0 or x == GRID_SIZE - 1 or y == GRID_SIZE - 1:
            gameMap[x][y] = 'X'
        elif abs(x - y) > 1 and random.randint(1, 100) <= 30:
            gameMap[x][y] = 'X'
        elif abs(x - y) > 1 and random.randint(1, 100) <= 5:
            gameMap[x][y] = 'V'
        elif abs(x - y) > 1 and random.randint(1, 100) <= 5:
            gameMap[x][y] = 'D'
...
# start position
red_x, red_y = GRID_SIZE - 14, GRID_SIZE - 14
blue_x, blue_y = 14, 14
...
def changingField(x, y):
    ...
    if gameMap[x][y] == "V":
        ...
        subprocess.run(["C:\\Program Files\\Python312\\python.exe", "test.py"])
        playerdata[f'p{t}'][f'p{t}attack'] += (1 if int(playerdata[f'p{t}'][f'p{t}attack'] * 0.1) == 0 else int(playerdata[f'p{t}'][f'p{t}attack'] * 0.1))
    elif gameMap[x][y] == "D":
        gameMap[x][y] = 0
        playerdata[f'p{t}'][f'p{t}defense'] += (1 if int(playerdata[f'p{t}'][f'p{t}defense'] * 0.1) == 0 else int(playerdata[f'p{t}'][f'p{t}defense'] * 0.1))
...
while playerdata[f'p1'][f'p1health'] > 0 and playerdata[f'p2'][f'p2health'] > 0:
    ...
    for x in range(GRID_SIZE):
        for y in range(GRID_SIZE):
            ...
            # drawing D
            if color == (0, 0, 0) and gameMap[x][y] == 'D':
                text = font.render('D', True, (60, 20, 220))
                text_rect = text.get_rect(center=(y * CELL_SIZE + CELL_SIZE // 2 + 180, x * CELL_SIZE + CELL_SIZE // 2 + 50))
                screen.blit(text, text_rect)
